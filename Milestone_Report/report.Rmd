...
To build a model, we don't need to use all the data at a time. We relatively use few randomly selected rows or chunks to represent for the entire data set. Since the data set is quite large, we only use a sub-sample data set of 10% of the whole data set.

```{r}
set.seed(42)
data <- c(sample(blogs, length(blogs)*0.1),
...
-   Uni-gram matrix with the threshold of 450 times:

```{r}
uni.gram.threshold <- findFreqTerms(uni.gram.matrix, lowfreq=450)
...
-   Bi-gram matrix with the threshold of 400 times:

```{r}
# only keep the words that more occur
bi.gram.threshold <- findFreqTerms(bi.gram.matrix, lowfreq=400)
...
-   Tri-gram matrix with the threshold of 150 times:

```{r}
# only keep the words that more occur
tri.gram.threshold <- findFreqTerms(tri.gram.matrix, lowfreq=150)
